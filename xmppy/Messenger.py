...
class Client(sleekxmpp.ClientXMPP):
    ...
    def receiveMessage(cls, jid, password, reply):
        ...
        instance = cls(jid, password, None, None, reply,
                       freq=-1, msg_t=False, rpl_t=True)
        return instance
    ...
    def sendMessage(cls, jid, password, recipient, message, freq=-1):
        ...
        instance = cls(jid, password, recipient, message,
                       None, freq, msg_t=True, rpl_t=False)
        return instance
    ...
    def initialize(cls, jid, password, recipient, message, reply, freq=10, wait=False):
        ...
        instance = cls(jid, password, recipient, message, reply,
                       freq=freq, msg_t=True, rpl_t=True, wait=wait)
        return instance
    ...
    def GPG(func):
        def wrap(self, *args, **kwargs):
            if self.gpg_encryption:
                message = self.gpg.encrypt(
                    func(self, *args, **kwargs), uids=self.uid)
            else:
