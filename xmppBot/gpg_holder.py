...
class Gpg:
    def __init__(self, gnupghome=''):
        ...
        if len(gnupghome)==0:
            ...
            log.debug('Creating gnupg instance in {}'.format("default directory."))
            self.gpg = gnupg.GPG()
            return None
        else:
            ...
            if os.path.exists(gnupghome):
                ...
                log.warning("path {} does not exist. Trying to create...".format(gnupghome))
                try:
                        ...
                        if type(_dict[keyword])==type(list()):
                            for value in _dict[keyword]:
                                if kwargs[keyword] in value:
                                    log.debug('Match in {}.'.format(_dict[keyword]))
                                    if not _dict in results: results.append(_dict)
                        else:
    ...
    def encrypt(self, message, sign=None, file=None, **kwargs):
        ...
        try:
            ...
            for key in keys:
                if key['ownertrust'] =='-':
                    log.warning("Key {} is untrusted. Forcing...".format(key['uids']))
                _encrypted = self.gpg.encrypt(str(message), key['keyid'], sign=sign, always_trust=True)
